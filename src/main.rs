...
fn next_byte(reg: &mut Registers, mem: &mut [u8]) -> u8 {
...
fn next_word(reg: &mut Registers, mem: &[u8]) -> u16 {
...
fn call_instruction(opcode: u8, reg: &mut Registers, mem: &mut [u8]) -> u32 {
    match opcode {
        ...
        0x20 => {
            if !reg.get_flag(Flag::Z) {
                let n = next_byte(reg, mem) as i32;
                reg.pc = ((reg.pc as i32) + n) as u16;
            }
            2
        },
